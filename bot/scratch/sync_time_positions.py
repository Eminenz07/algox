...
# Sync time offset with Bybit server to prevent 10002 future timestamp errors
...
try:
    ...
    print(f"Time offset calculated: {offset:.3f} seconds")
    # Monkeypatch time.time to adjust for drift
    time.time = lambda: original_time() + offset
except Exception as e:
    print("Could not synchronize time with Bybit:", e)
    ...
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        ...
        if user:
            ...
            if creds:
                ...
                # Query Bybit directly using pybit
